...
eventlistener.addEventListener("msg", e => {
  processmsg(arg.data);
})

function processmsg(g) {
  let arg = {msg:g}
  let e, link
...
function validtoken(bot, guildInfo) {
  if (bot && guildInfo) {
...
id("msgin").addEventListener("keypress", e => {
  if (!istyping) {
    typing(true, currentchannel)
    istyping = true
  }
  if (id("msgin").value === "") {
    typing(false, currentchannel)
    istyping = false
  }
  ...
  typing(false, currentchannel)
  istyping = false;
  msgin(id('msgin').value, currentchannel)
  id('msgin').value = ""
})
...
//                  this was previously ps.js
// -----------------------------------------------------------------

/**
 * 
 * @param {Boolean} ss true to start, false to stop
 * @param {Number} chid channel id to be typing in
 */
function typing(ss, chid) {
  if (!Rsettings.typing) return;
  if (ss) {
    client.channels.cache.get(chid).startTyping()
  } else if (!ss) {
    client.channels.cache.get(chid).stopTyping()
  }
}
...
function process(msg, iscached = false) {
  ...
  if (Rsettings.csenabled && !iscached) {
    ...
    scriptreader.update() // is the event above supposed to do this? was missing an s idk
...
function msgin(msg, channel) {
  let arg = {msg, channel}; // so i dont have to replace all instances :grimace:
  let tagx = /@.*#[0-9]{4}/;
...
function init(tok) {
  ...
  client.on("messageUpdate", (oldMessage, newMessage) => {
    ...
    messageUpdate(oldMessage.id, newMessage.content)
  })
  client.login(tok)
    .catch(err => {
  ...
  // both of these status events are from status.html so they get to stay
  ipcRenderer.on("clearstatus", () => {
    client.user.setActivity(null);
    document.querySelector(".status-pres").innerHTML = 'Set a status'
    clearmodule("./settings.json")
  ...
  ipcRenderer.on("statusentered", (event, arg) => {
    // pain
    clearmodule("./settings.json")
    let newsettings = require("./settings.json")
